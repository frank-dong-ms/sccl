...
def register_ndv4_plans():
    ...
    @register_sccl_program(fully_connected(8), 'alltoall', 'ndv4', protocol='LL128', sizes=('1MB', '32MB'), machines=lambda x: x == 2 or x == 4 or x == 8 or x == 16 or x == 32)
    def ndv4_alltoall(prog, nodes):
        alltoall_hierarchical(num_nodes=nodes, gpus_per_node=8)

    @register_sccl_program(fully_connected(8), 'alltoall', 'ndv4', protocol='Simple', sizes=('32MB', None), machines=lambda x: x == 2 or x == 4 or x == 8 or x == 16 or x == 32)
    def ndv4_alltoall(prog, nodes):
        alltoall_hierarchical(num_nodes=nodes, gpus_per_node=8)

    @register_sccl_program(fully_connected(8), 'alltoall', 'ndv4', protocol='LL128', sizes=('1MB', '32MB'), machines=lambda x: x == 2 or x == 4 or x == 64)
    def ndv4_alltoall(prog, nodes):
        alltoall_hierarchical(num_nodes=nodes, gpus_per_node=8)

    @register_sccl_program(fully_connected(8), 'alltoall', 'ndv4', protocol='Simple', sizes=('32MB', None), machines=lambda x: x == 2 or x == 4 or x == 64)
    def ndv4_alltoall(prog, nodes):
